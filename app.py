...
class FloatVocabDB:
    ...
    def save_plan(self, lexicon_id: int, daily_new: int, target_date: str, alpha: float, font_size: int, bg_color: str):
        self.conn.execute(
            """
            UPDATE plans
            SET lexicon_id = ?, daily_new = ?, target_date = ?, float_alpha = ?,
                font_size = ?, bg_color = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = 1
            """,
            (lexicon_id, daily_new, target_date, alpha, font_size, bg_color),
        )
        self.conn.commit()
...
class FloatingWindow(tk.Toplevel):
    def __init__(self, app):
        ...
        self.flipped = False
        self.withdraw()
        self.title("FloatVocab")
        self.attributes("-topmost", True)
        self.overrideredirect(False)
        self.geometry("360x210+960+120")
        self.configure(padx=16, pady=14)

        self.lexicon_label = tk.Label(self, text="", anchor="w")
        self.lexicon_label.pack(fill="x")
        self.word_label = tk.Label(self, text="", font=("Segoe UI", 28, "bold"), wraplength=320)
        self.word_label.pack(expand=True, fill="both")
        self.detail_label = tk.Label(self, text="", wraplength=320, justify="center")
        self.detail_label.pack(fill="x")
        self.hint_label = tk.Label(self, text="点击翻面 · ← 不认识 · → 认识", font=("Segoe UI", 9))
        self.hint_label.pack(fill="x", pady=(8, 0))

        self.bind("<Button-1>", lambda _event: self.flip())
    ...
    def apply_style(self):
        ...
        for widget in [self.lexicon_label, self.word_label, self.detail_label, self.hint_label]:
    ...
    def render(self):
        if not self.card:
            self.lexicon_label.configure(text="今日完成")
            self.word_label.configure(text="没有待背单词")
            self.detail_label.configure(text="可以回主窗口导入新词库，或者明天再来。")
            return
        self.lexicon_label.configure(text=f"{self.card.lexicon_name} · {status_text(self.card.status)}")
        if self.flipped:
            ...
            self.detail_label.configure(text=self.card.phonetic)
...
class FloatVocabApp:
    ...
    def build_ui(self):
        ...
        ttk.Button(style_box, text="选择颜色", command=self.choose_color).grid(row=3, column=0, sticky="ew", pady=8)
        ttk.Button(style_box, text="应用样式", command=self.save_plan).grid(row=3, column=1, sticky="ew", pady=8)
        ttk.Label(style_box, text="全局快捷键：Alt+Space 翻面，Alt+Left 不认识，Alt+Right 认识").grid(row=4, column=0, columnspan=2, sticky="w")
